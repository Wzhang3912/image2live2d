    ...
    def _start(self, log):
        ...
        for _ in range(30):  # service already confirmed healthy; just wait for the forward to bind
            try:
                ...
                self._up = True; self.status = "up"; log("GPU ready")
                return
...
def _run_pipeline(job: _Job, data: bytes, filename: str) -> None:
    ...
    def s_decompose():
        ...
        if ext == ".psd":
            psd = work / f"{job.name}.psd"; psd.write_bytes(data)
            ctx["stack"] = decompose.from_psd(psd, work / "layers")
        elif ext == ".zip":
            layers = work / "layers"; layers.mkdir(parents=True, exist_ok=True)
            with zipfile.ZipFile(io.BytesIO(data)) as zf:
                pngs = [n for n in zf.namelist() if n.lower().endswith(".png") and not n.endswith("/")]
                if not pngs:
                    raise ValueError("zip contains no PNG layers")
            ...
            img = work / f"{job.name}{ext}"; img.write_bytes(data)
    ...
    def s_stop_gpu():
        if ctx.get("gpu") and not ctx.get("gpu_released"):
            ctx["gpu"].release(); ctx["gpu_released"] = True
...
def live2d_bundle_dir(job: "_Job") -> Path | None:
    ...
    physics_file = None
    if rig.physics:
        physics_file = "model.physics3.json"; wj(physics_file, _physics3.physics3(rig))
    cdi_file = "model.cdi3.json"; wj(cdi_file, _cdi3.cdi3(rig))
    motions = {}
    for anim in rig.animations:
        rel = f"model.{anim.name}.motion3.json"; wj(rel, _motion3.motion3(anim))
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            ...
            if len(parts) >= 3 and parts[0] == "api" and parts[1] == "jobs":
                job = get_job(parts[2])
                if job is None:
                    self._json(404, {"error": "no such job"}); return
                sub = parts[3] if len(parts) > 3 else ""
                if sub == "":
                    self._json(200, job.public()); return
                if sub == "download":
                    if not job.inp_bytes:
                        self._json(409, {"error": "not ready"}); return
                    ...
                    self.end_headers(); self.wfile.write(job.inp_bytes); return
                if sub == "source":
                    if not job.source:
                        self._json(404, {"error": "no source"}); return
                    self._send(200, job.source, "image/png"); return
                if sub == "params":
                    self._json(200, {"params": preview_param_specs(job)}); return
                if sub == "rig":
                    if job.params is None:
                        self._json(409, {"error": "rig not ready"}); return
                    self._json(200, rig_json(job)); return
                ...
                if sub == "tex":  # /api/jobs/{id}/tex/{i}
                    try:
                        ...
                        self._json(400, {"error": "bad texture index"}); return
                    op = _ordered_parts(job)
                    if not (0 <= i < len(op)):
                        self._json(404, {"error": "no such part"}); return
                    self._send(200, Path(op[i][0].texture_path).read_bytes(), "image/png"); return
                if sub == "render":
                    if job.params is None:
                        self._json(409, {"error": "rig not ready"}); return
                    q = parse_qs(u.query)
